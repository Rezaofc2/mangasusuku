...
export default function Page() {
  ...
  async function tryEndpoint() {
    ...
    try {
      ...
      // Jika response diawali dengan <!DOCTYPE, berarti HTML (kemungkinan diblokir firewall/WAF Sucuri)
      if (text.trim().startsWith("<!DOCTYPE") || text.trim().startsWith("<html")) {
        setError("Request diblokir oleh Web Application Firewall (WAF) Sucuri Mangasusu karena mengandung karakter sensitif. Mohon gunakan query pencarian kata biasa (contoh: 'milf' atau 'hotel') tanpa menyisipkan tag HTML/karakter berbahaya.");
        setResponse(text);
        return;
      }
  ...
  const curlExample = (() => {
    ...
    endpoint.params.forEach((p) => {
      // Ganti placeholder tag HTML `<...>` agar tidak diblokir Sucuri Firewall
      if (p.required) {
        url.searchParams.set(p.name, p.name === "q" ? "milf" : p.name);
      }
    });
